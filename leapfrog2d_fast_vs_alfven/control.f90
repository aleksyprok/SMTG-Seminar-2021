...
CONTAINS
  ...
  SUBROUTINE set_initial_conditions
    ...
    DO iz = 0, nz
      DO iy = 0, ny + 1
        u_perp1( iy,iz) = 2.0_num * dome(yc(iy), zb(iz))
        u_perp1m(iy,iz) = 2.0_num * dome(yc(iy), zb(iz))
      END DO
    END DO

    DO iz = 0, nz + 1
      DO iy = 0, ny
        u_perp2( iy,iz) = 2.0_num * dome(yb(iy), zc(iz))
        u_perp2m(iy,iz) = 2.0_num * dome(yb(iy), zc(iz))
      END DO
    END DO

    DO iz = 0, nz + 1
      DO iy = 0, ny + 1
        ux1( iy,iz) = 2.0_num * dome(yc(iy), zc(iz))
        ux1m(iy,iz) = 2.0_num * dome(yc(iy), zc(iz))
      END DO
    END DO

    DO iz = 0, nz
      DO iy = 0, ny
        ux2( iy,iz) = 2.0_num * dome(yb(iy), zb(iz))
        ux2m(iy,iz) = 2.0_num * dome(yb(iy), zb(iz))
      END DO
    END DO
