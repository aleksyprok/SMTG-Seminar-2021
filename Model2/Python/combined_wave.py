...
def combined_wave(y, z, t, var):
    for n in range(3):
        if var == 'ux':
            if n == 0:
                wave = ux0[0] * np.exp(1j * (ky * y + kz[0] * z + omega * t))
            else:
                wave += ux0[n] * np.exp(1j * (ky * y + kz[n] * z + omega * t))
        elif var == 'u_perp':
            if n == 0:
                wave = u_perp0[0] * np.exp(1j * (ky * y + kz[0] * z + omega * t))
            else:
                wave += u_perp0[n] * np.exp(1j * (ky * y + kz[n] * z + omega * t))
        elif var == 'bx':
            if n == 0:
                wave = bx0[0] * np.exp(1j * (ky * y + kz[0] * z + omega * t))
            else:
                wave += bx0[n] * np.exp(1j * (ky * y + kz[n] * z + omega * t))
        elif var == 'b_perp':
            if n == 0:
                wave = b_perp0[0] * np.exp(1j * (ky * y + kz[0] * z + omega * t))
            else:
                wave += b_perp0[n] * np.exp(1j * (ky * y + kz[n] * z + omega * t))
        elif var == 'b_par':
            if n == 0:
                wave = b_par0[0] * np.exp(1j * (ky * y + kz[0] * z + omega * t))
            else:
                wave += b_par0[n] * np.exp(1j * (ky * y + kz[n] * z + omega * t))
        else:
...
print(np.amax(np.abs([ux0[2], u_perp0[2], bx0[2], b_perp0[2], b_par0[2]])))
